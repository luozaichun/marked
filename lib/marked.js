...
// span level renderer
    ...
    Parser.prototype.parse = function (src) {
        var me = this, tocItems = '', last = 0;
        //预生成好目录标签
        src.forEach(function (token) {
            if (token.type == 'heading') {
                id = token.text.toLowerCase();
                if(token.depth>last&&last!=0){
                    var _ul='<ul>'+me.renderer.tocItem(id, token.depth, token.text);
                    tocItems+=_ul
                    last=token.depth;
                }else if(token.depth<last&&last!=0){
                    var closeEle='';
                    for(var k=0;k<last-token.depth;k++){
                        closeEle+='</ul>'
                    }
                    tocItems +=closeEle+'</li>'+me.renderer.tocItem(id, token.depth, token.text)+'</li>';
                    last=token.depth;
                }else if(token.depth&&last==0){
